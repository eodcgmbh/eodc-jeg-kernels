...
logger = logging.getLogger("server_listener for Julia launcher")
logger.setLevel(log_level)
fileHandler = logging.FileHandler("{0}/{1}.log".format("/tmp", "server_listener.log"))
...
def _select_ports(count, lower_port, upper_port):
    ...
    for _ in range(count):
        ...
        sockets.append(sock)
        with open("/tmp/info.txt", "a") as file:
            file.write("Selected and opened port: " + str(sock.getsockname()[1]) + "\n")
    with open("/tmp/info.txt", "a") as file:
            file.write(f"Selected and opened {count} ports")
    for sock in sockets:
        with open("/tmp/info.txt", "a") as file:
            file.write("Trying to close socket " + str(sock.getsockname()[1]) + "\n")
        sock.close()

    with open("/tmp/info.txt", "a") as file:
            file.write("Returning ports\n")
    return ports
...
#def setup_server_listener(conn_filename):
def setup_server_listener(
    conn_filename, parent_pid, lower_port, upper_port, response_addr, kernel_id, public_key
):
    with open("/tmp/info.txt", "w") as file:
        file.write("Setting up server listener\n")
    ...
    response_address = response_addr #os.getenv('RESPONSE_ADDRESS')
    #public_key = os.getenv('PUBLIC_KEY')
    #port_range_array = port_range.split('..')
    lower_port = 49152 #port_range_array[0]
    upper_port = 65535 #port_range_array[1]

    #with open("/tmp/info.txt", "a") as file:
    #    file.write("Selecting ports\n")
    ports = _select_ports(5, lower_port, upper_port)

    #with open("/tmp/info.txt", "a") as file:
    #    file.write(f"conn_filename: {conn_filename}\n")
    ...
    print(f"Connection file written to: {conn_filename}")
    if response_address:
        ...
        if comm_socket:
            print(f"Server listener started for kernel {kernel_id}")
